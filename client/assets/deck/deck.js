...
var Deck = (function () {
  ...
  var poker = {
    deck: function deck(_deck3) {
      ...
      function poker(next, count, cb, max) {
        var cards = _deck3.cards;
        var len = cards.length;

        cards.slice(-count).reverse().forEach(function (card, i) {
          card.poker(i, count, max, len, function (i) {
            cb(card, i)
            if (i === count - 1) {
              next();
            }
    ...
    card: function card(_card3) {
      ...
      _card3.poker = function (i, count, maxOneRow = 4, len, cb) {
        var delay = i * 250;
        var target = {
          x: (i % maxOneRow - maxOneRow / 2 + .5) * 110 | 0,
          y: -(7.5 * 16) * (Math.ceil(count / maxOneRow) - Math.floor(i / maxOneRow)) - 10
        };
        ...
        setTimeout(function () {
          ...
          $el.style[transform] = translate(target.x + '%', target.y + 'px');
          // console.log(translate(target.x + '%', target.y + '%'))
        }, delay + 25);
  ...
  var rotatePoker = {
    deck: function deck(_deck6) {
      _deck6.rotatePoker = _deck6.queued(rotatePoker);

      function rotatePoker(next) {
        var cards = _deck6.cards;

        cards.forEach(function (card) {
          card.rotatePoker(function (i) {
            if (i === cards.length - 1) {
              next();
            }
          });
    ...
    card: function card(_card6) {
      ...
      _card6.rotatePoker = function (cb) {
        var i = _card6.i;
        var delay = i * 10;
        var posX = -(6.75 - value) * 15;
        var posY = -(1.5 - suit) * 105;
  ...
  function Card(i) {
    ...
    var $el = createElement('div', 'card', 
      createElement('div', 'wrap', createElement('div', 'face'), createElement('div', 'back'))
    );
    // var $topleft = createElement('div');
    // var $bottomright = createElement('div');
    // var $face = createElement('div', 'face');
    // var $back = createElement('div', 'back');
  ...
  Deck.modules = { bysuit: bysuit, fan: fan, intro: intro, poker: poker, shuffle: shuffle, sort: sort, rotatePoker: rotatePoker };
  Deck.Card = Card;
  Deck.easing = easing;
  Deck.prefix = prefix;
